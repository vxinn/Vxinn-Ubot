    ...
    if HEROKU_API_KEY is not None:
        ...
        if HEROKU_APP_NAME is None:
            await event.edit(
                ...
                " untuk dapat deploy perubahan terbaru dari ✨Kᴀʏᴢᴜ-Uʙᴏᴛ✨.`"
            )
            repo.__del__()
            return
        ...
        if heroku_app is None:
            await event.edit(
                f"{txt}\n`Kredensial Heroku tidak valid untuk deploy Kayzu-Project dyno.`"
            )
            return repo.__del__()
        await event.edit(
...
async def upstream(event):
    ...
    if changelog == "" and force_update is False:
        await event.edit(
            f"\n✨Kᴀʏᴢᴜ-Uʙᴏᴛ✨ Sudah Versi Terbaru || Tunggu Update Terbaru\n"
        )
        await asyncio.sleep(15)
        await event.delete()
        return repo.__del__()
